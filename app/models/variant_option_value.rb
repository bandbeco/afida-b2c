...
# Join table connecting ProductVariant to ProductOptionValue
# Enables proper normalization of option data with support for display labels
#
# == Schema
#   product_variant_id       - The variant this option assignment belongs to
...
#
# == Constraints
#   - One value per option type per variant (unique on variant_id + option_id)
#   - Prevents duplicate exact assignments (unique on variant_id + value_id)
#
class VariantOptionValue < ApplicationRecord
  ...
  # Ensure only one value per option type per variant
