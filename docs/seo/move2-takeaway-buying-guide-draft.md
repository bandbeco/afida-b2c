---
type: Draft
description: Buying guide (~2,050 words) for /collections/takeaway with sub-category links woven in.
status: shipped
timestamp: 2026-06-19
---

# `/collections/takeaway` buying guide — DRAFT for review

**Status:** ✅ APPLIED TO PRODUCTION 2026-06-19 (8,349 chars written via kamal to `Collection.regular.find_by(slug: "takeaway").buying_guide`; verified live over HTTP — guide, all 6 category links, and Article JSON-LD render). Approved by Laurent with sub-category links woven in. `**bold**` stripped to honor the no-bold rule (the vegware guide uses bold but that predates/violates the rule; not propagated).
**Target queries:** "takeaway packaging suppliers" (328, pos 18), "eco friendly takeaway containers", "biodegradable takeaway containers", "takeaway boxes".
**Voice/structure:** matches the existing restaurants/coffee-shops guides (## Key Factors → ### subsections → ## Pro Tips → ## Summary). Specs grounded in the live takeaway collection product mix (kraft trays/bowls 500-1000ml, soup cups 12/16oz, handle bags, pizza boxes 7-16in, takeaway boxes No.1/No.8, wooden cutlery). ~2,050 words.
...
- Order free samples before you commit. Samples are free and you only pay delivery, so test the bowls, boxes, and bags with your actual dishes and a real delivery run before you buy a pallet. [Order a free sample pack](/samples).
...
A well-stocked takeaway carries a small number of container families matched to its food, the right takeaway box and pizza box sizes for its menu, strong handle bags sized to the largest order, and compliant wooden cutlery supplied only when needed. Buy by the case, consolidate the high-turnover lines to keep the unit cost down, and stock a microwave-safe option for reheaters. Get the match between container and dish right and the rest of the operation, cost, presentation, and the customer's experience at the door, follows. Not sure where to start? [Order free samples](/samples) of the containers and bags you are considering, or browse the full takeaway range below.

---

## Note for review
- All product specifics (kraft bowls 500-1000ml, soup cups 12/16oz, takeaway boxes No.1 755ml / No.8 1300ml, pizza boxes 7/9/12/16in, twisted + flat handle bags, 6.5in wooden cutlery) are taken from the live `/collections/takeaway` product mix.
- **Internal links woven in (recommended + applied):** 6 contextual links to the nested category pages, one per section on the keyword phrase — `/categories/hot-food/food-containers`, `/categories/hot-food/soup-containers`, `/categories/hot-food/takeaway-boxes`, `/categories/hot-food/pizza-boxes`, `/categories/bags-and-wraps/bags`, `/categories/tableware/cutlery` — plus 2 `/samples` links. All targets verified to resolve. This passes link equity to the buried sub-category pages (the Move 2 targets) and gives readers a direct path to buy. Pattern matches the vegware guide, which already links to its sub-pages; the renderer allows in-content links (confirmed). Kept to one link per section to avoid spamminess.
- ~2,050 words, vs restaurants 9,142 chars (~1,500 words) and coffee-shops ~10,347 chars. In range.
