...
Progress 2026-09-03: the code is built on `agentic-commerce-plan` and not yet deployed. `Webhooks::AgenticCheckoutController` at `POST /webhooks/stripe/agentic_checkout` verifies the Agentic Commerce Extension signature and renders `AgenticCommerce::CheckoutCustomization`, a pure function of the hook request that reuses `Shipping` and `ShippingZone` (free over the mainland threshold, the off-mainland charge for Highlands and islands, no option for non-GB or Crown Dependency addresses, the cached UK VAT rate on every line item unless `automatic_tax.enabled`). `Checkout::AgentOrderCreator` builds the order from `line_items[].price.external_reference`; `Webhooks::StripeController` routes sessions without `cart_id` metadata to it, and an unknown SKU is a permanent failure (200 to Stripe, Sentry alert). `orders.source` (`web` | `agent`) and `orders.agent_name` were added. Because the Klaviyo "Placed Order" event rides `order.placed`, which only the success redirect emitted, the webhook now emits it for agent orders. The same gap still exists for website orders the webhook rebuilds after a missed redirect; that is pre-existing and left alone here; tracked in [issue #294](https://github.com/bandbeco/afida-b2c/issues/294).

Review follow-up 2026-09-07: an `xhigh` code review of the branch found the routing rule "no `cart_id` means agent" broke every other cart-less session (Payment Links, Dashboard sessions, sessions predating the metadata), which used to produce an itemless order plus ops alerts and now died inside the agent creator as a 500 and 72h of Stripe retries. Routing is now positive — a session is an agent session when its line items carry catalogue `external_reference`s — and anything else falls back to the itemless order as before. Also fixed: line items with no `external_reference` (delivery, fees) are skipped rather than aborting the order as an unknown SKU; the creator refuses to commit a paid order with no items; `customer_details` is read safely so a missing one fails validation instead of raising; `Checkout::MissingShippingDetails` and the new `SessionNotBuildableError` are in the webhook's permanent-failure list rather than falling through to a retry storm; the hook verifies the signature with Stripe's default timestamp tolerance (it was skipping the freshness check, so a captured request could be replayed forever) and rejects requests outright when the secret is missing instead of answering 200 with no VAT and no delivery; the customization hook withholds the UK VAT rates as well as the shipping option for addresses we do not deliver to; line-item paging moved to `Checkout::SessionLineItems.all` so the agent and web money paths share one copy; and `order.placed` now carries `order.source` from both emitters.

Deployed 2026-09-03: the hook is configured in the live account at `https://afida.com/webhooks/stripe/agentic_checkout` with Custom shipping options on and Order approvals off, `stripe.agentic_hook_secret` is in credentials, and production rejects unsigned calls with 400. The Dashboard wizard offered no Custom tax rates toggle, but Settings → Agentic commerce has one and it is on, so hook requests should arrive with `automatic_tax.enabled` false and the hook's UK VAT rate applies to every line item. Still to do before the exit criteria can be checked: the Dashboard test purchase. Two things only the sandbox can answer: whether an empty `shipping_options` array declines a non-GB checkout, and whether the shipping option is taxed at all under manual tax rates (the website taxes delivery by sending it as a line item, which the hook cannot do; if Stripe leaves delivery untaxed, the order still records exactly what was charged, but VAT on delivery is under-collected and the fix is a shipping `tax_code` plus Stripe Tax, or accepting it).
...
- Resolved 2026-09-03: a non-GB address is declined before payment. The Workbench "Trigger Agentic Purchase" blueprint (live mode; its address form offers the US only) failed at "Initiating checkout" with `[Seller error] invalid_parameter: Shipping is unavailable for the provided address`, so the order approval hook is not needed to block non-GB. Workbench → Extension runs showed no run at all, so the refusal came from the feed's GB-only shipping rule and the hook was never called: the feed is the first gate on country. The hook's empty `shipping_options` path therefore only matters for GB-coded addresses we do not deliver to (Channel Islands and Isle of Man postcodes) and is still unexercised in live.
- The Workbench test tool cannot exercise the GB happy path (US addresses only), so the exit criteria for a GB order remain unverified until either Stripe confirms a way to test with a GB address or the first real agent order arrives and is reconciled by hand.
...
- The feed quotes one flat `GB:ALL:Standard:1-1:6.99 GBP` rule while the hook charges `Shipping.cost_for_zone` (£25 off-mainland) and promises 2-4 days, so a Highlands, islands or Northern Ireland buyer sees the price and the delivery date change between the agent's listing and checkout. This is not fixable in the feed today: re-read on 2026-09-07, the spec's `delivery_area` accepts a postal code or postal-code range "currently supported only for the US", leaving `region` (an ISO 3166-2 subdivision code) or `ALL` for GB, and the zone table is postcode-derived precisely because local-authority boundaries are not the same thing (see `ShippingZone`). `free_shipping_threshold` has the same gap: it takes `region` only, so the mainland-only free-delivery rule is advertised country-wide. The hook stays authoritative and the order records what was charged. Revisit when Stripe extends postal-code delivery areas beyond the US; the interim options are a single GB-wide price at the off-mainland rate (overcharging the mainland) or `shipping_exclusions` for the off-mainland areas (refusing those orders outright), and neither is better than the current mismatch.
