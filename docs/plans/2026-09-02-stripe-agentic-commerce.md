...
- Resolved 2026-09-03: a non-GB address is declined before payment. The Workbench "Trigger Agentic Purchase" blueprint (live mode; its address form offers the US only) failed at "Initiating checkout" with `[Seller error] invalid_parameter: Shipping is unavailable for the provided address`, so the order approval hook is not needed to block non-GB. Whether the refusal came from the hook's empty `shipping_options` or from the feed's GB-only shipping rule is visible in Workbench → Extension runs.
