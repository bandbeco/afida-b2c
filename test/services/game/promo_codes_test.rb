...
class Game::PromoCodesTest < ActiveSupport::TestCase
  ...
  test "mints a single-use STACK code drawing on the month's win coupon, expiring with the month" do
    Stripe::Coupon.stubs(:retrieve).with("afida-stack-ten-#{MONTH}").returns(stub(id: "afida-stack-ten-#{MONTH}"))
    captured = nil
    ...
    assert_equal({ type: "coupon", coupon: "afida-stack-ten-#{MONTH}" }, captured[:promotion])
  ...
  test "mints MATE codes against the month's referral coupon" do
    Stripe::Coupon.stubs(:retrieve).with("afida-stack-ref-#{MONTH}").returns(stub(id: "afida-stack-ref-#{MONTH}"))
    captured = nil
    ...
    assert_equal({ type: "coupon", coupon: "afida-stack-ref-#{MONTH}" }, captured[:promotion])
    assert_match(/\AMATE[#{Game::PromoCodes::CODE_ALPHABET.join}]{6}\z/, captured[:code])
  end

  test "creates the coupon on first use as ten pounds off, capped so a month's giveaway is bounded" do
    Stripe::Coupon.stubs(:retrieve).raises(Stripe::InvalidRequestError.new("No such coupon", "coupon"))
    captured = nil
    Stripe::Coupon.stubs(:create).with { |p| captured = p; true }.returns(stub(id: "afida-stack-ten-#{MONTH}"))
    Stripe::PromotionCode.stubs(:create).returns(stub(code: "STACKJKX397"))

    assert_equal "STACKJKX397", Game::PromoCodes.mint_win_code

    assert_equal "afida-stack-ten-#{MONTH}", captured[:id]
    assert_equal 1000, captured[:amount_off]
    assert_equal "gbp", captured[:currency]
    assert_nil captured[:percent_off]
