...
class OrderPricingDisplayIntegrationTest < ActionDispatch::IntegrationTest
  setup do
    @user = users(:one)
    @order = orders(:one)
  ...
  test "order show page displays pack pricing format for pack-priced items" do
  ...
  test "order show page displays correct line total" do
    order_item = OrderItem.create!(
      order: @order,
      product: products(:one),
