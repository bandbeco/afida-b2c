class PopulateVegwareSuppliesAndEssentialsBuyingGuide < ActiveRecord::Migration[8.1]
  def up
    collection = Collection.find_by(slug: "vegware")
    category = Category.find_by(slug: "supplies-and-essentials", parent_id: nil)
    return unless collection && category

    guide = File.read(Rails.root.join("lib/data/collections/buying-guides/vegware/supplies-and-essentials.md"))
