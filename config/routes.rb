Rails.application.routes.draw do
  ...
  # June 2026 admin category restructure: renamed slugs 301 via the DB slug
  # history (CategorySlugRedirect, backfilled in migration 20260708123508)
  # through the categories/collections/samples controllers, including the
  # unknown-children-of-renamed-parents fallback. One mapping cannot live
  # there: the old "hot-food/food-containers" child slug is now the live slug
  # of its renamed parent, so a slug-history row would be shadowed by the live
  # category. It stays as a static route (and its old slug must therefore
  # never be handed to another category; see Category::RESERVED_REDIRECT_SLUGS).
  get "/categories/hot-food/food-containers", to: redirect(status: 301) { |_params, req| "/categories/food-containers/food-containers-and-lids#{req.query_string.present? ? "?#{req.query_string}" : ""}" }
